...
try:
    ...
    class SEQ_ACE_RIDGE(Solver):

        def __init__(self, name, pt, config):
            super().__init__(name, pt, config)

        def return_reg(self,alval,enforce_pos):
            if bool(enforce_pos):
                reg = Ridge(alpha = alval, positive=bool(enforce_pos),solver='lbfgs', fit_intercept = False)
            else:
                reg = Ridge(alpha = alval, positive=bool(enforce_pos), fit_intercept = False)
            return reg
        ...
        def perform_fit(self, a=None, b=None, w=None, fs_dict=None, trainall=False):
            ...
            if pt._rank == 0:
                ...
                if 'EXTRAS' in self.config.sections and self.config.sections['EXTRAS'].apply_transpose:
                    bw = aw.T @ bw
                    aw = aw.T @ aw
                assert 'ACE' in list(self.config.sections.keys()), "sequential ridge fits are only implemented with ACE"
                ...
                alvals = self.config.sections['SEQ_ACE_RIDGE'].alpha
                if len(self.config.sections['SEQ_ACE_RIDGE'].enforce_pos_rank) > 1:
                    enforce_pos_rank = [ int(ki) for ki in self.config.sections['SEQ_ACE_RIDGE'].enforce_pos_rank]
                else:
                    enforce_pos_rank = [ int(ki) for ki in self.config.sections['SEQ_ACE_RIDGE'].enforce_pos_rank] * len(these_lens)
